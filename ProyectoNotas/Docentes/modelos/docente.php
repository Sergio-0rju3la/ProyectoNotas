...
public function getidad($IDdoc){
    ...
    public function updatead($IDdoc,$NombreDoc,$ApellidoDoc,$DocumentoDoc,$CorreoDoc,$MateriaDoc,$PerfilDoc,$EstadoDoc)
    {
        ...
        if($statement->execute())
        {
            header('Location: ../../Administrador/pages/index.php');
            session_start();
            $_SESSION['alerta']='<div id="alerta"class="alert alert-success" role"alert">El docente de id '.$IDmat.' ha sido actualizado</div>';
        }else{
            echo "el usuario no";
    ...
    public function deletead($IDdoc){
        ...
        if($statement->execute()){
            ...
            header('location: ../../Administrador/pages/index.php');
            session_start();
            $_SESSION['alerta']='<div id="alerta"class="alert alert-danger" role"alert">Docente eliminado</div>';
        }else{
